...
export function CommandHeader() {
  return (
    <header className="sticky top-0 z-10 flex items-center gap-3 border-b border-border/40 bg-background/60 px-5 py-3 backdrop-blur-xl">
      ...
      <div className="relative ml-4 flex-1 max-w-2xl">
        ...
        <input
          ...
          className="h-10 w-full rounded-lg border border-border/60 bg-input/50 pl-9 pr-28 text-sm outline-none placeholder:text-muted-foreground/70 focus:border-primary/60 focus:ring-2 focus:ring-primary/30"
        />
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <select className="rounded-md border border-border/60 bg-secondary/40 px-2 py-1.5 text-xs">
          {districts.map((d) => <option key={d}>{d}</option>)}
        </select>
        <select className="rounded-md border border-border/60 bg-secondary/40 px-2 py-1.5 text-xs">
          <option>All severities</option><option>Critical</option><option>High</option><option>Medium</option><option>Low</option>
        </select>
        <button className="relative grid h-9 w-9 place-items-center rounded-md border border-border/60 bg-secondary/40 hover:text-primary">
