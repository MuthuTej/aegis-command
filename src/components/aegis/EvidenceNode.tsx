...
export function VictimNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={[
      ...
      <Handle type="source" position={Position.Left}   className="!h-2 !w-2 !border-0 !bg-cyan-400" />
...
export function SuspectNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={[
      "relative rounded-xl border-2 px-3 py-2.5 min-w-[185px] backdrop-blur-xl",
      ...
      <Handle type="target" position={Position.Left}   className="!h-1.5 !w-1.5 !border-0 !bg-red-400" />
      <Handle type="source" position={Position.Bottom} className="!h-1.5 !w-1.5 !border-0 !bg-red-400" />
    </div>
  );
}
...
export function ZoneNode({ data, selected }: NodeProps) {
  ...
  const meta = NODE_META[data.type] ?? NODE_META.witness;
  const Icon = meta.icon;
  return (
    <div className={[
      "relative rounded-xl border px-3 py-2 min-w-[170px] backdrop-blur-xl",
      "bg-slate-950/75",
      z.ring,
      ...
      <div className="flex items-center gap-2 mb-1.5">
        ...
        {data.danger && <ShieldAlert className="h-3 w-3 text-red-400 shrink-0 animate-pulse" />}
      </div>
      {data.sublabel && (
      ...
      {/* Activity pulse for danger nodes */}
      {data.danger && (
...
// ── Legacy EvidenceNode wrapper (kept for backward compat) ────────────────────
...
// ── Activity indicator node (for AI overlay) ──────────────────────────────────
export { Activity };
